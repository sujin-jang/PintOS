...
struct inode_disk
  {
    /** Data sectors */
    disk_sector_t direct_blocks[DIRECT_BLOCKS_COUNT];
...
struct inode_indirect_block_sector {
  disk_sector_t blocks[INDIRECT_BLOCKS_PER_SECTOR];
};
...
static bool
inode_reserve_indirect (disk_sector_t* p_entry, size_t num_sectors, int level)
{
...
static void
inode_deallocate_indirect (disk_sector_t entry, size_t num_sectors, int level)
{
  // only supports 2-level indirect block scheme as of now
  ASSERT (level <= 2);
